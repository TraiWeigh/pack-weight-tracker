 ...
 *
 * References Part 18 requirement numbers where applicable.
 */
...
function makeCol(id, name, photoCount = 0) {
  const photos = Array.from({ length: photoCount }, (_, i) => ({
    id: `p${i}-${id}`, dataUrl: `data:image/jpeg;base64,photo${i}${id}`,
  }));
  return { id, name, photos };
}

function makePhoto(id, suffix = '') {
  return { id, dataUrl: `data:image/jpeg;base64,${suffix || id}` };
}

// ── A1: Dropdown constants ────────────────────────────────────────────────────
// Requirements 1–3 are UI rendering; data-layer analogue: verify Landscapes is
// always separate from custom themes and Add Theme is controlled by count.

describe('A1 — Dropdown data structure', () => {
  it('built-in Landscapes is separate from custom collections', () => {
    // Landscapes is not stored in the collections array — it is a hardcoded
    // UI entry.  Custom collections can coexist without conflict.
    const cols = [makeCol('c1', 'Sierra'), makeCol('c2', 'Desert')];
    // Neither collection shadows a "landscapes" entry
  ...
  it('Add Theme is available when fewer than MAX_COLLECTIONS custom themes exist', () => {
    ...
    // Simulating UI logic: canAddTheme = cols.length < MAX_COLLECTIONS
  ...
  it('Landscapes does not count toward the MAX_COLLECTIONS limit', () => {
    // A user with 10 custom themes plus Landscapes still only counts 10 custom
    const customCount = MAX_COLLECTIONS;
    // There is no landscape in the collections array; the check is purely on
    // collections.length, which excludes Landscapes
...
describe('A2 — Add Theme name validation (Req 6, 8, 9)', () => {
  ...
  it('Req 7: custom theme label format is "Theme [Collection Name]" (verified in UI)', () => {
    // Data-layer analogue: the name "Test" should produce label "Theme Test"
    // when prefixed in the UI.
    const res = createCollection('Test', []);
    assert.ok(res !== null);
...
describe('A3 — Theme count limits (Req 13, 14, 15)', () => {
  ...
  it('Req 14: Landscapes (built-in) does not occupy a slot in the limit count', () => {
    // The limit applies only to collections[] length, not including Landscapes
    const cols = Array.from({ length: MAX_COLLECTIONS }, (_, i) =>
      makeCol(`c${i}`, `T${i}`),
    );
    // Can still add a landscape photo; canAddTheme check is collections.length only
...
describe('A4 — Photos per theme (Req 16, 17, 18)', () => {
  ...
  it('Req 17: photo is appended (slot-filling behaviour)', () => {
    ...
    const photo = makePhoto('p1', 'data1');
...
describe('A5 — Prompt 016 migration (Req 21, 22)', () => {
  it('Req 21: existing Prompt 016 collections are preserved by runMigration', () => {
    // A pre-existing collection (from Prompt 016 separate Add Collection area)
    // should survive migration unmodified if no legacy single-photo slot exists
  ...
  it('Req 22: migration with existing collections is idempotent', () => {
    const dataUrl = 'data:image/jpeg;base64,legacyPhoto';
    // Use the exact dataUrl string (not through makePhoto helper which would double-prefix)
    const myPhotos = { id: 'mp1', name: 'My Photos', photos: [{ id: 'p1', dataUrl }] };
    const cols = [myPhotos, makeCol('c1', 'Test Themes')];
    const result = runMigration(cols, dataUrl); // same dataUrl — idempotency: should not add again
    assert.equal(result.find(c => c.name === 'My Photos')?.photos.length, 1, 'No duplicate');
    assert.equal(result.find(c => c.name === 'Test Themes') !== undefined, true, 'Custom theme preserved');
  });

  it('Req 22: running migration twice never duplicates a photo', () => {
    const dataUrl = 'data:image/jpeg;base64,onlyOnce';
    let cols = [];
    cols = runMigration(cols, dataUrl, 'pid1', 'cid1');
    cols = runMigration(cols, dataUrl, 'pid2'); // second run — same photo
...
describe('A6 — Rename preserves photos (Req 23)', () => {
  it('Req 23: renaming a theme preserves all photo references', () => {
    const photos = [makePhoto('p1', 'a'), makePhoto('p2', 'b')];
    const cols = [{ id: 'c1', name: 'Old', photos }];
    const result = renameCollection('c1', 'Sierra', cols);
    assert.ok(result !== null);
    assert.equal(result[0].name, 'Sierra');
    assert.equal(result[0].photos.length, 2);
    assert.equal(result[0].photos[0].dataUrl, 'data:image/jpeg;base64,a');
    assert.equal(result[0].photos[1].dataUrl, 'data:image/jpeg;base64,b');
  });

  it('Req 23: renaming a theme preserves photo order', () => {
    const photos = [makePhoto('p1', 'first'), makePhoto('p2', 'second'), makePhoto('p3', 'third')];
    const cols = [{ id: 'c1', name: 'Test', photos }];
...
describe('A7 — Delete theme data behaviour (Req 24)', () => {
  it('Req 24: deleting the active theme removes it from the collections array', () => {
    ...
    // UI must then fall back to Landscapes — that is state logic in BackgroundPicker
  });

  it('Req 24: other themes are fully unaffected by deletion', () => {
    const photos = [makePhoto('p1', 'keep')];
    const cols = [
...
describe('A8 — Data persists through JSON serialisation (Req 25)', () => {
  it('Req 25: collections survive a JSON stringify/parse round-trip', () => {
    const photos = [makePhoto('p1', 'photo1'), makePhoto('p2', 'photo2')];
    ...
    assert.equal(restored[0].photos[0].dataUrl, 'data:image/jpeg;base64,photo1');
  });
