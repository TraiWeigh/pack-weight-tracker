...
- Name: phase1d/mobile-v3-sheets.spec.ts >> Transition timing and reduced motion >> NavDrawer: CSS close duration is 300 ms (Tailwind data-[state=closed]:duration-300)
- Location: tests/e2e/phase1d/mobile-v3-sheets.spec.ts:468:7
...
- generic:
  ...
  - dialog [ref=e2]:
    ...
    - generic [ref=e10]:
      - generic [ref=e11]:
        - generic [ref=e12]: Units
        - generic [ref=e13]:
          - button "imperial" [pressed] [ref=e14] [cursor=pointer]
          - button "metric" [ref=e15] [cursor=pointer]
      - button "Checklist — Trail checklist for selected items" [ref=e17] [cursor=pointer]:
        - generic [ref=e23]:
          - generic [ref=e24]: Checklist
          - generic [ref=e25]: Trail checklist for selected items
      - button "Print — Print your gear list as PDF" [ref=e26] [cursor=pointer]:
        - generic [ref=e32]:
          - generic [ref=e33]: Print
          - generic [ref=e34]: Print your gear list as PDF
```

# Test source

```ts
  381 |         const backBtn = page.locator('button[aria-label="Back"]');
  382 |         await expect(backBtn).toBeVisible({ timeout: 3_000 });
  383 | 
  384 |         // Content text matching the page is present somewhere on screen
  385 |         await expect(page.locator(`text=${row.contentSignal}`).first()).toBeVisible({ timeout: 3_000 });
  386 |       });
  387 |     }
  388 | 
  389 |     test('"Sources & References" dismisses sheet and opens sources screen (not footer-page)', async ({ page }) => {
  390 |       await openMore(page);
  391 |       const dialog = getOpenDialog(page);
  392 | 
  393 |       const rowBtn = dialog.locator('button').filter({ hasText: 'Sources & References' }).first();
  394 |       await expect(rowBtn).toBeVisible();
  395 |       await rowBtn.click();
  396 | 
  397 |       // Sheet must close
  398 |       await page.waitForSelector('[role="dialog"]', { state: 'hidden', timeout: 2_000 });
  399 | 
  400 |       // Sources screen: has a "Back" button and "Sources" text
  401 |       await expect(page.locator('button[aria-label="Back"]')).toBeVisible({ timeout: 3_000 });
  402 |       await expect(page.locator('text=Sources').first()).toBeVisible({ timeout: 3_000 });
  403 |     });
  404 | 
