 ...
 *
 * Chromium validates the invariant geometry and R0095 internal-checklist-scroll behavior.
...
test.describe('R0092 — lock Bottom Box Groups and align to weight master line', () => {
  for (const viewport of [
    ...
    test(`preserves the master line and protected geometry at ${viewport.width}×${viewport.height}`, async ({ page, errors }) => {
      ...
      expect(viewportData.footerPositioning, 'R0095 footer must use the stable shell position').toBe('shell-top-static');
      expect(viewportData.footerPosition, 'footer must remain an absolute shell layer').toBe('absolute');
  ...
  test('keeps the Bottom Box Groups viewport-stable while checklist content scrolls and clears the final row', async ({ page, errors }) => {
    await page.setViewportSize({ width: 402, height: 714 });
    await page.goto('/mobile-functional-v3');
    await page.waitForSelector('.tw-v3-root');

    // Add enough normal categories to require checklist scrolling without touching
    ...
    const beforeScroll = await page.evaluate(() => (document.querySelector('[data-testid="main-scroll"]') as HTMLElement).scrollTop);
    ...
    const scroll = await page.evaluate(() => ({
      y: (document.querySelector('[data-testid="main-scroll"]') as HTMLElement).scrollTop,
      max: Math.max(0, (document.querySelector('[data-testid="main-scroll"]') as HTMLElement).scrollHeight - (document.querySelector('[data-testid="main-scroll"]') as HTMLElement).clientHeight),
      windowY: window.scrollY,
      rootPaddingBottom: getComputedStyle(document.querySelector('.tw-v3-root')!).paddingBottom,
    }));

    expect(scroll.y, 'checklist scroll must remain available').toBeGreaterThan(beforeScroll + 50);
    expect(scroll.max, 'scenario must have a meaningful checklist scroll range').toBeGreaterThan(50);
    expect(scroll.windowY, 'ordinary checklist scrolling must not move the document').toBe(0);
    expect(Math.abs(after.top - before.top), 'Bottom Box Groups top must remain stable during checklist scroll').toBeLessThanOrEqual(1);
    expect(Math.abs(after.bottom - before.bottom), 'Bottom Box Groups bottom must remain stable during checklist scroll').toBeLessThanOrEqual(1);
