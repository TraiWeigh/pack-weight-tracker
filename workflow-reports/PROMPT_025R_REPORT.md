...
## ADDENDUM — DATABASE-VERIFIED ROOT CAUSE
### photoId Identity, Storage Location, Resolution Paths, and Divergence Proof

**Added after DB query confirmed:** `background = {"type": "custom", "photoId": "701cc0ea-4912-416c-b08e-0c747381668c"}`

---

### A1. DB Query Evidence

```sql
SELECT id, name, saved_at,
       payload->'background' AS background_field,
       jsonb_typeof(payload->'background') AS bg_json_type
FROM locker_entries
WHERE name ILIKE '%sample%' OR name ILIKE '%live%' OR name ILIKE '%test%'
ORDER BY saved_at DESC LIMIT 10;
```

**Result:**

| id | name | saved_at | background_field | bg_json_type |
|----|------|----------|-----------------|--------------|
| ed2e848c-7154-48b6-b0a8-b8f3d6016f89 | Sample List Live Test | 2026-08-12 06:33:27.203+00 | `{"type": "custom", "photoId": "701cc0ea-4912-416c-b08e-0c747381668c"}` | object |
| test-file-B-023P | Test File B | 2026-08-10 20:23:20.245+00 | null | null |
| test-file-A-023P | Test File A | 2026-08-10 20:23:20.244+00 | null | null |

**Key confirmed values:**
- Locker entry id: `ed2e848c-7154-48b6-b0a8-b8f3d6016f89`
- background type: `"custom"`
- photoId: `"701cc0ea-4912-416c-b08e-0c747381668c"`
- DB JSONB field is populated — NOT null

---

### A2. What the photoId References

**`701cc0ea-4912-416c-b08e-0c747381668c`** is a **UUID v4** (36 characters, 4 hyphen groups: 8-4-4-4-12).

Comparison to built-in Landscape preset photoIds (from `BackgroundPicker.tsx:90–99`):

| Preset name | photoId format | Example value |
|-------------|---------------|---------------|
| Rocky Mountains | Unsplash numeric-alphanumeric | `1464822759023-fed622ff2c3b` |
| Swiss Alps | Unsplash numeric-alphanumeric | `1506905925346-21bda4d32df4` |
| Forest | Unsplash numeric-alphanumeric | `1448375240586-882707db888b` |
| … (all 10 presets) | same pattern | — |
| **Owner's background** | **UUID v4** | **`701cc0ea-4912-416c-b08e-0c747381668c`** |

**Conclusion:** The photoId is unambiguously a `crypto.randomUUID()` output — generated by the custom photo upload path in `BackgroundPicker.tsx:700`:
```ts
photoId = crypto.randomUUID();
await storePhoto(photoId, blob, mimeType, width, height);
```

This UUID does **not** match any Unsplash photo ID format and does **not** appear in the `PRESETS` array. It cannot resolve to a public CDN URL.

---

### A3. Where the Referenced Image Is Stored

**Sole location: Owner's browser IndexedDB**

```
Database name : trailweigh          (BG_DB_NAME, bgPhotoStore.ts:20)
Object store  : bgPhotos            (BG_DB_STORE, bgPhotoStore.ts:21)
Key path      : photoId             (bgPhotoStore.ts:48)
Record shape  : { photoId: string, blob: Blob, mimeType: string,
                  width: number, height: number }
```

The image blob for `701cc0ea-4912-416c-b08e-0c747381668c` lives **only** in `bgPhotos` inside the Owner's browser. There is no server-side copy, no CDN copy, no object storage entry. The blob was written by `storePhoto()` at upload time and has never left the Owner's device.

**The DB `locker_entries.payload.background` stores only the UUID reference string**, not the blob itself. The server has no knowledge of the image content.

---

### A4. Why a "Built-in Landscape Selection" Is Represented as type: custom

**It is not a built-in Landscape preset selection.** This is a critical finding.

Built-in Landscape presets, when selected, produce `{ type: 'preset', id: '<preset-slug>' }` — for example `{ type: 'preset', id: 'rocky-mountains' }`. This type is set in the picker's selection handler and stored in `localStorage['trailweigh:background']`.

The `type: 'custom'` representation is only created by **two paths**:

**Path 1 — Photo upload** (`BackgroundPicker.tsx:693–714`):
```ts
photoId = crypto.randomUUID();
await storePhoto(photoId, blob, mimeType, width, height);
const entry = { id: photoId };
// later: onBackgroundChange({ type: 'custom', photoId })
```
User uploads a JPEG/PNG/WebP to a Custom Theme slot. The blob is compressed and stored in IndexedDB. A UUID is assigned.

**Path 2 — Legacy migration** (`BackgroundPicker.tsx:384–402` + `bgPhotoStore.ts:350–360`):
```ts
// Old format found in localStorage: { type:'custom', dataUrl:'data:image/...' }
...
```
An older format background (`{ type: 'custom', dataUrl: '...' }`) that existed before the IndexedDB blob-store was introduced is migrated on first open of the BackgroundPickerPanel. The dataUrl is converted to a Blob, stored in IndexedDB, and replaced with the UUID reference.

**Most likely explanation for Owner:** The owner either (a) uploaded a landscape-style photo to a Custom Theme and selected it, or (b) had an older custom background (dataUrl format) that was automatically migrated to the UUID+IndexedDB format when they first opened the Background Picker after the migration was shipped.

In either case, what the owner **sees** appears like any other landscape photo — the display is indistinguishable. But the **storage representation** is fundamentally different from a built-in preset.

---

### A5. Is This Representation Intentional?

**YES — completely intentional and correct.**

The `type: 'preset'` vs `type: 'custom'` split is the core architectural privacy boundary of the background system:

| Type | Storage | Resolution | Cross-device | Cross-browser | Review-accessible |
|------|---------|-----------|-------------|--------------|------------------|
| `preset` | Unsplash CDN (public) | `getFullUrl(PRESETS.find(...).photoId)` → public HTTPS URL | YES | YES | YES |
| `custom` | Owner's browser IndexedDB | `getPhotoBlob(photoId)` → IndexedDB lookup → Blob → Object URL | NO | NO | NO |

The `type: 'custom'` representation correctly signals that the background cannot be shared cross-browser. There is no data corruption or mis-tagging here.

The 025Q guard at `ReviewPage.tsx:357–363` was explicitly written to enforce this boundary:
```ts
...
This comment was written at the time 025Q was applied — **the code already understood this exact case**. The guard is architecturally sound.

---

### A6. How Owner Resolves the photoId

**Full resolution chain (Owner's browser):**

1. `background` React state = `{ type: 'custom', photoId: '701cc0ea-4912-416c-b08e-0c747381668c' }` (from `localStorage['trailweigh:background']` on init, or from `handleLoadFromLocker` → `setBackground(entry.background)`)

2. `Checklist.tsx:872–875` — detects custom type:
   ```ts
   const activePhotoId =
     background?.type === 'custom'
       ? (background as { type: 'custom'; photoId: string }).photoId
       : null;
   // → activePhotoId = '701cc0ea-4912-416c-b08e-0c747381668c'
   ```

3. `Checklist.tsx:877–909` — `useEffect([activePhotoId])` fires:
   ```ts
   const blob = await getPhotoBlob(activePhotoId);
   // → openPhotoDb() → IndexedDB 'trailweigh' / 'bgPhotos'
   // → get('701cc0ea-4912-416c-b08e-0c747381668c')
   // → returns { photoId, blob, mimeType, width, height }
   // → blob = owner's compressed JPEG image
   ```

4. `URL.createObjectURL(blob)` → short-lived `blob:https://...` object URL

5. `setCustomBgObjectUrl(url)` → `bgImageUrl = customBgObjectUrl` (`Checklist.tsx:914`)

6. `div.screen-only` receives inline style:
   ```
   backgroundImage: linear-gradient(...),url(blob:https://... )
   ```
   Owner sees the landscape photo. **Works because the IndexedDB blob exists in Owner's browser.**

---

### A7. Whether Signed-out Review Can Resolve the Same photoId

**NO. Categorically impossible with current architecture.**

**Full resolution chain (Reviewer's browser):**

1. DTO returned from `GET /api/links/${token}`:
   ```json
   { "background": { "type": "custom", "photoId": "701cc0ea-4912-416c-b08e-0c747381668c" }, ... }
   ```
   Server correctly reads this from `locker_entries.payload.background`. Data is present and correct.

2. `seedFromLiveFiles()` executes (CASE A or C):
   ```ts
   const bg = primary.background as { type?: string } | null;
   // bg = { type: 'custom', photoId: '701cc0ea-...' }
   if (bg?.type === 'preset') { ... }   // FALSE — does not enter
   else {
     localStorage.removeItem('trailweigh:background');  // KEY IS REMOVED
   }
   ```
   `trailweigh:background` is removed from reviewer's localStorage. **This is correct behavior.**

3. `setStatus('ready')` → `ChecklistContent` mounts → `background` useState initializer runs:
   - `tw-fork-id` → NOT set (review mode)
   - `tw-savedlist-bg` → NOT set (review mode)
   - Normal path: `localStorage.getItem('trailweigh:background')` → **null** (was just removed)
   - `background` React state initializes as **null**

4. `activePhotoId = null` (since `background = null`)
   `useEffect([activePhotoId])` → clears `customBgObjectUrl` → null
   `bgImageUrl = null`

5. `div.screen-only` receives **no** `backgroundImage` inline style.
   Reviewer sees **white background** (the `bg-background` CSS class).

**Even if CASE A is bypassed (CASE B — returning reviewer):**
The reviewer's IndexedDB `trailweigh / bgPhotos` still does not contain the blob for `701cc0ea-...`. Even if `trailweigh:background` were somehow written to the reviewer's localStorage with the correct `{ type: 'custom', photoId: '701cc0ea-...' }` value, `getPhotoBlob('701cc0ea-...')` → IndexedDB lookup → `req.result = undefined` → returns `null` → `customBgObjectUrl = null` → `bgImageUrl = null` → white background.

**There is no code path by which the reviewer can display this image without:**
a. Server-side blob storage (object storage, per-user, with appropriate auth/URL generation), OR
b. The owner switching to a built-in Landscape preset and saving

---

### A8. Is This the First Proven Divergence Causing Review to Show White?

**YES — this is the definitive, database-confirmed, code-traced first divergence.**

Prior hypotheses in the main 025R report (CASE B returning reviewer, null DB payload, timing issues) were all secondary or conditional. This finding supersedes them all:

| Hypothesis | Status | Evidence |
|------------|--------|---------|
| DB payload null (background never saved) | **DISPROVEN** | DB query confirms `background_field` is populated non-null |
| CASE B (returning reviewer, SV match) | **Secondary** | Even CASE A produces white — the photoId UUID is unresolvable in any browser but Owner's |
| Timing gap (025Q writes after mount) | **DISPROVEN** | 025Q writes happen before mount; timing was always correct |
| 022G startup effect gated on userId | **Secondary** | Even if 022G ran in review mode, `getPhotoBlob` would return null |
| **025Q guard rejects type:custom** | **CONFIRMED PRIMARY CAUSE** | Guard correctly removes `trailweigh:background` when type ≠ 'preset'. Design is correct. The background is unresolvable cross-browser. |

**The chain of proof:**

```
DB payload:    { type: 'custom', photoId: '701cc0ea-...' }   [DB query confirmed]
        ↓
Server DTO:    background = { type: 'custom', photoId: '701cc0ea-...' }   [links.ts:98]
        ↓
025Q guard:    bg?.type === 'preset' → FALSE
               → localStorage.removeItem('trailweigh:background')   [ReviewPage.tsx:362]
        ↓
Initializer:   localStorage.getItem('trailweigh:background') → null   [Checklist.tsx:~330]
        ↓
background:    null   [React state]
        ↓
bgImageUrl:    null   [Checklist.tsx:911]
        ↓
DOM:           no backgroundImage style → white background   [Checklist.tsx:2014]
        ↓
IndexedDB:     getPhotoBlob('701cc0ea-...') → null (blob not in reviewer's DB)
               [bgPhotoStore.ts:79–89]
               [Even if background state were set — still unresolvable]
```

**The 025Q fix was WORKING CORRECTLY.** It correctly identified the custom-type background and correctly refused to expose a device-local blob reference to the reviewer. The behavior "Review shows white background" is the architecturally correct response to a custom background that cannot cross the device boundary.

---

### A9. Revised Assessment of 025Q

**025Q was NOT a failure.** The user-verified FAIL verdict was based on the observable outcome (white background in Review) — which is correct behavior when the owner's background is type:custom. 025Q executed perfectly:

1. Detected `type: 'custom'` ✓
2. Refused to write an unresolvable blob reference ✓
3. Removed the global key (to prevent a stale reviewer-owned background from showing) ✓
4. Left Review with a null background ✓

The product gap is not in the code — it is in the **user expectation**: the owner's background appears in their browser because their IndexedDB has the blob, but the system cannot share that blob to Review without a new capability (server-side photo storage).

**The 025Q code comment (ReviewPage.tsx:354–356) already fully documented this:**
> "Custom backgrounds ({type:'custom',photoId}) reference blobs in the OWNER'S IndexedDB — not available in the reviewer's browser. Only preset backgrounds (bundled Unsplash images) are safe to apply globally."

This was written at the time 025Q was applied, meaning the engineer who wrote 025Q understood the limitation and chose to handle it correctly. The "failure" was in the test expectation, not the code.

---

### A10. Implications and Recommended Next Steps

**Immediate (no code change):**
- The owner can switch their background from the custom photo to one of the 10 built-in Landscape presets, click Save, and the background WILL appear in Review. This requires no code change.
- The owner should be informed: "Custom uploaded photos cannot appear in shared Review links. Use a built-in Landscape preset for backgrounds that share."

**Near-term product gap (existing task #29 — "Make your background show up when someone views your shared list"):**
Task #29 now requires an architectural decision, not just a code fix:
- **Option A (User education, zero code):** Add a notice in the share dialog or background picker when a custom photo is active: "Custom photos won't appear in shared links. Switch to a Landscape preset to share your background." Smallest scope.
- **Option B (Server-side blob storage, large scope):** Store user-uploaded background blobs in object storage (per user, authenticated). Serve a short-lived signed URL in the DTO for Review to fetch. Large scope; requires object storage, per-user blob management, cleanup logic.
- **Option C (Preset-only sharing, medium scope):** When generating a share link, if the background is custom, snap a copy of the nearest Landscape preset match, or store null. The owner sees their custom photo; Review sees null or a preset approximation.

**Task #28 ("Let users pick a solid colour or gradient") is now even more relevant:**
Solid colours and gradients require no blob storage — they are CSS values. A solid colour background would be shareable in Review without any architecture change. This path simultaneously addresses the sharing gap and adds a lighter-weight customisation option.

**Correction to main 025R report section "LEADING HYPOTHESIS FOR WHY 025Q FAILED":**
The hypotheses in the compacted session summary were all superseded. The correct explanation is A10 above. The "025Q failure" was a correct function of the privacy guard, not a bug.

---

### A11. Evidence Summary Table

| Question | Answer | Evidence source |
|----------|--------|----------------|
| What does photoId reference? | UUID v4 key for blob in Owner's browser IndexedDB `trailweigh/bgPhotos` | ID format analysis; bgPhotoStore.ts:61–76 |
| Where is the image stored? | Owner's browser IndexedDB only — no server copy | bgPhotoStore.ts:20–21; lockerApi.ts PUT body |
| Why is Landscape shown as type:custom? | It is NOT a built-in preset. It is a user-uploaded or migrated custom photo | BackgroundPicker.tsx:51–52, 693–714 |
| Is the representation intentional? | YES — type:custom correctly marks device-local private blobs | BackgroundPicker.tsx architecture; bgPhotoStore.ts design |
| How does Owner resolve it? | IndexedDB lookup → Blob → createObjectURL → inline backgroundImage | Checklist.tsx:872–914 |
| Can Review resolve it? | NO — reviewer's IndexedDB has no blob for this UUID | bgPhotoStore.ts:79–89; Checklist.tsx:911–914 |
| First proven divergence? | YES — the 025Q guard correctly removes the key; the blob is unresolvable cross-browser | ReviewPage.tsx:357–363; DB query; bgPhotoStore.ts |
| Was 025Q a code failure? | NO — 025Q correctly handled this case and its own comment documents it | ReviewPage.tsx:354–356 (comment) |

---

### A12. Application Code Changes in This Addendum

**NONE.** This addendum is read-only diagnostic work.

`git diff --name-only HEAD` — no change from end of main 025R body.

Files read: `artifacts/pack-checklist/src/lib/bgPhotoStore.ts`, `artifacts/pack-checklist/src/components/BackgroundPicker.tsx` (lines 384–420, 693–714), `artifacts/pack-checklist/src/pages/ReviewPage.tsx` (full), `artifacts/pack-checklist/src/pages/Checklist.tsx` (lines 868–916), `artifacts/api-server/src/routes/links.ts` (previously read).

DB queries run: 1 read-only SELECT (development environment).
